...
로봇 수가 늘수록 충돌 트리가 지수적으로 분기해 단일 경로 계획 호출 비용이 급증, 60fps 시뮬레이션 루프(16ms/frame)의 예산을 초과했습니다. 브라우저 메인 스레드가 블로킹되어 시뮬레이션이 사실상 멈췄습니다.
...
결과적으로 실시간 프레임 예산 안에서 동작하면서 교착을 크게 감소시켰습니다.`,
    result: '채택 — 실시간 프레임 예산 내 동작 + 교착 대폭 감소',
    resultColor: '#3fb950',
  },
  {
    phase: '5단계',
    ...
    desc: `Priority A* + WHCA* 하이브리드로 교착을 크게 줄였지만, 로봇을 수십 대로 늘리자 잔여 교착이 다시 시스템을 멈췄습니다. 원인을 추적한 결과 근본 문제는 경로탐색 알고리즘이 아니라 두 가지 구조적 결함이었습니다.
...
const TECHNIQUES = [
  ...
  },
  {
    ...
    solution: '최소 힙(MinHeap) 자료구조로 open 집합을 관리하는 공용 코어(aStarCore)로 6개 알고리즘 전부 교체. Dijkstra·Greedy·Stochastic·Priority·WHCA*가 동일 코어를 재사용.',
...
function CbsDeepDive() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <p style={{ fontSize: 12, color: '#8b949e', lineHeight: 1.7, margin: '0 0 4px' }}>
        CBS(충돌 트리)는 이론상 최적이지만 실시간에 비싸므로, 본 프로젝트는 <strong style={{ color: '#e6edf3' }}>별개 계열인 WHCA*(예약 테이블 기반 단일 패스 A*)</strong>를 채택했습니다. 아래는 두 방식의 대비입니다.
      </p>
      {/* 개념 설명 */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div style={{ background: '#21262d', borderLeft: '3px solid #58a6ff', borderRadius: 8, padding: 16 }}>
          <div style={{ fontWeight: 700, color: '#58a6ff', fontSize: 13, marginBottom: 12 }}>WHCA* (현재 채택)</div>
          {[
            'CBS의 충돌 트리(CT)를 쓰지 않는 예약 테이블 방식 (WHCA* 계열)',
            '예약 테이블로 충돌 노드 비용 ×8',
      ...
      <div style={{ background: '#21262d', borderRadius: 8, padding: 16 }}>
        ...
        <div style={{ fontFamily: 'monospace', fontSize: 11, color: '#8b949e', lineHeight: 2 }}>
          ...
          <div style={{ marginTop: 8, color: '#3fb950' }}>【WHCA* — 예약 테이블 방식】</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <Card>
        <div style={{ fontSize: 13, color: '#8b949e', lineHeight: 1.85 }}>
          하이브리드 알고리즘으로 교착을 크게 줄였지만, 로봇을 수십 대 이상으로 늘리자 잔여 교착이 다시 전체를 멈췄습니다.
        ...
        <Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[
              ...
              { icon: '⚡', title: 'CPU 부하 최소화', desc: '60fps 프레임 예산 내 동작. 로봇 수에 무관하게 일정한 연산량.', color: '#58a6ff' },
      ...
      <section>
        ...
        <Card>
          ...
          <blockquote style={{ borderLeft: '3px solid #58a6ff', paddingLeft: 14, margin: '18px 0 0', fontSize: 12, color: '#8b949e', lineHeight: 1.7 }}>
            WHCA*는 4지표 중 3개가 Priority A*보다 우수합니다. 그럼에도 <strong style={{ color: '#e6edf3' }}>보조 채택</strong>인 이유는 (1) CPU 효율이 상대적으로 낮아 프레임 예산을 위협하고,
            (2) 모든 에이전트가 갱신하는 <strong style={{ color: '#e6edf3' }}>공유 예약 테이블</strong>에 의존해 단독 동작이 불가능하기 때문입니다. 실제 채택 구조는
            <strong style={{ color: '#3fb950' }}> Priority A* (기본 경로계획) + WHCA* (예약 충돌 시 ×8 페널티 보강)</strong> 의 <strong>층화 하이브리드</strong>입니다.
          </blockquote>
        </Card>
      </section>
